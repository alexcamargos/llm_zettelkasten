...
O linter de integridade do cofre pode ser executado diretamente pelo terminal via `uv run zettelbrain-lint` ou pelo alias curto `uv run zb-lint` (ou `uv run zettelbrain-lint --json` para saídas estruturadas) para validar a saúde de links mortos, órfãos, ligação mínima ao grafo, referências a deprecados e padrões de conhecimento emergentes no ZettelBrain. Para baixar e limpar artigos web automaticamente a partir de um link, execute `uv run zb-article-etl --url "<URL>"` (ou `uv run zettelbrain-article-etl`), que salvará o Markdown limpo em `raw/articles/`. Para validar o servidor sem iniciar o transporte MCP, rode `uv run python src/mcp/server.py --health-json`.
